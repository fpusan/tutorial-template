...
More details on this can be found on the `SQMtools paper <https://bmcbioinformatics.biomedcentral.com/articles/10.1186/s12859-020-03703-2>`_. Our intentions here are to include several standard normalization methods so our users can easily pick the ones that suit them the best. At the same time, we understand that novice users can find all these options overwhelming, and would rather prefer to just have the "best" way given to them. While we don't want to push so hard towards any particular methods (since this is anyways a field that is constantly evolving) we have set the defaults of many functions in *SQMtools* to reflect what we currently believe to the the best (or at least safest) approach. Below we discuss our current view on the matter (which is subject to change as we too keep learning better ways of doing things):
